    ...
    <div id="content-wrapper" class="layout">
       ...
       <div class="page-container" style="padding-left: 0px">
			<div class="main-content">
				...
				@include('configuracion.insurers.oficce.store')
				@include('configuracion.insurers.oficce.view')
				@include('configuracion.insurers.oficce.edit')
...
<script>
			...
			function update(){
				enviarFormularioPut("#form-update", 'api/insurers/oficce', '#cuadro4', false, "#avatar-edit");
			}


			function store(){
				enviarFormulario("#store", 'api/insurers/oficce/', '#cuadro2');
			}
			...
			function list(cuadro) {
				...
				var table=$("#table").DataTable({
					...
					"ajax":{
						"method":"GET",
						 "url":''+url+'/api/insurers/oficce/'+{{$id_surgerie}},
						 "data": {
							"id_user": id_user,
							"token"  : tokens,
						},
			...
			*/
			function ver(tbody, table){
				...
				$(tbody).on("click", "span.consultar", function(){
					...
					$("#fax_view").val(data.fax).attr("disabled", "disabled")
					$("#name_contact_view").val(data.name_contact).attr("disabled", "disabled")
					$("#phone_contact_view").val(data.phone_contact).attr("disabled", "disabled")
					$("#email_contact_view").val(data.email_contact).attr("disabled", "disabled")
				
					data.headquarters_default  == 1 ? $("#headquarters_default_view").prop("checked", true)   : $("#headquarters_default_view").prop("checked", false) 
			...
			*/
			function edit(tbody, table){
				$(tbody).on("click", "span.editar", function(){
					...
					$("#name_contact_edit").val(data.name_contact)
					$("#phone_contact_edit").val(data.phone_contact)
					$("#email_contact_edit").val(data.email_contact)
				
					data.headquarters_default  == 1 ? $("#headquarters_default_edit").prop("checked", true)   : $("#headquarters_default_edit").prop("checked", false) 

					$("#id_edit").val(data.id_insurers_oficce)
			...
			*/
			function desactivar(tbody, table){
				$(tbody).on("click", "span.desactivar", function(){
					var data=table.row($(this).parents("tr")).data();
					statusConfirmacion('api/insurers/oficce/status/'+data.id_insurers_oficce+"/"+2,"¿Está seguro de desactivar el registro?", 'desactivar');
				});
			}
		/* ------------------------------------------------------------------------------- */
			...
			*/
			function activar(tbody, table){
				$(tbody).on("click", "span.activar", function(){
					var data=table.row($(this).parents("tr")).data();
					statusConfirmacion('api/insurers/oficce/status/'+data.id_insurers_oficce+"/"+1,"¿Está seguro de desactivar el registro?", 'activar');
				});
			}
			...
			function eliminar(tbody, table){
				$(tbody).on("click", "span.eliminar", function(){
					var data=table.row($(this).parents("tr")).data();
					statusConfirmacion('api/insurers/oficce/status/'+data.id_insurers_oficce+"/"+0,"¿Esta seguro de eliminar el registro?", 'Eliminar');
				});
			}
